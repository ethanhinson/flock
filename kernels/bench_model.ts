...
async function batched(
  ...
  const build = (p: GPUComputePassEncoder) => {
    for (let i = 0; i < reps; i++) dispatch(p);
  };
...
console.log(
  `  weights read in ${(loadMs / 1000).toFixed(1)}s; token_embd repack+upload ` +
...
console.log("embedding gather (Q8_0 row -> f32), batched per dispatch");
{
  ...
  for (const n of [1, 16, 64, 256]) {
    ...
    const us = (await batched(
      (p) => {
        p.setPipeline(embedPipe);
        p.setBindGroup(0, bg);
    ...
    console.log(
      `  ${String(n).padStart(3)} token(s): ${us.toFixed(2).padStart(7)} us` +
...
console.log("LM head: hidden (1024) x token_embd^T (151936), batched per dispatch");
{
  ...
  const us = (await batched(
    (p) => {
      p.setPipeline(mvPipe);
      p.setBindGroup(0, bg);
      p.dispatchWorkgroups(groups, 1);
    },
    logits,
    200,
    fenceMs,
  ...
  console.log(
    `  151936x1024: ${us.toFixed(1)} us   ${gflops.toFixed(0)} GFLOP/s   ` +
      `${gbs.toFixed(0)} GB/s of weight traffic   (${groups} workgroups)`,
  );
  console.log(
    `  ${(bytes / 1e6).toFixed(0)} MB of weights read per call -- the whole tied matrix,`,
  );
  ...
  console.log(
    `  For scale: ~${(us / 37).toFixed(0)}x the biggest single op inside a layer, and ` +
  ...
  const sus = (await batched(
    (p) => {
      p.setPipeline(mvPipe);
      p.setBindGroup(0, sbg);
  ...
  console.log(
    `  same kernel at 3072x1024: ${sus.toFixed(1)} us, ${sGbs.toFixed(0)} GB/s ` +
      `(${(sBytes / 1e6).toFixed(1)} MB)`,
  );
  console.log(
    `  byte rate at the head's shape vs the layer's shape: ` +
      `${(gbs / sGbs).toFixed(2)}x.`,
  );
  if (gbs / sGbs > 1.1) {
    console.log(
      `  -> The head is ${(us / sus).toFixed(0)}x slower than the layer shape for ${
        (VOCAB / smallRows).toFixed(0)
      }x the rows, so`,
    );
    console.log(
      `     it runs ${(gbs / sGbs).toFixed(1)}x MORE efficiently per byte, not less. The layer`,
    );
    ...
    console.log(
      `  -> Same byte rate at both shapes, so the ${(us / sus).toFixed(0)}x cost is the ${
        (VOCAB / smallRows).toFixed(0)
      }x problem`,
    );
  ...
  console.log(
    `  Whether ${gbs.toFixed(0)} GB/s is near this machine's ceiling is NOT measured, so no`,
  );
...
console.log("argmax over 151936 logits: GPU reduction vs host readback");
{
  ...
  console.log(
    `  reduction, batched:  pass1 ${us1.toFixed(2)} us + pass2 ${us2.toFixed(2)} us ` +
  ...
  const reduceMs = await wall(async () => {
    ...
    a.setPipeline(p1);
    a.setBindGroup(0, bg1);
    a.dispatchWorkgroups(GROUPS);
    a.end();
    const b = enc.beginComputePass();
    b.setPipeline(p2);
    b.setBindGroup(0, bg2);
    b.dispatchWorkgroups(1);
    b.end();
    const rd = dev.createBuffer({
      size: 4,
      usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST,
  ...
  console.log(
    `    copy ${(VOCAB * 4 / 1e6).toFixed(2)} MB back + scan in JS:   ${readbackMs.toFixed(2)} ms`,
  );
  ...
  console.log(
    `  backend regardless of size, so it swamps the ${
      (VOCAB * 4 / 1e6).toFixed(2)
    } MB of copy and the`,
  );
  console.log(
    `  ${(us1 + us2).toFixed(0)} us of kernel alike. The reduction is still the right choice --`,
  );
  ...
  console.log(
    `  the ${
      (readbackMs / reduceMs).toFixed(2)
    }x is the honest number, not a speedup worth citing.\n`,
  );
...
console.log("whole-model timings (NOT batched: this is what a generation pays)\n");
{
  ...
  for (const n of [1, 8, 16, 64, 128, 256]) {
    ...
    console.log(
      `    ${String(n).padStart(3)} tokens: ${ms.toFixed(1).padStart(7)} ms   ` +
        `${(ms / n).toFixed(2).padStart(6)} ms/token   ${
          (n / (ms / 1000)).toFixed(0).padStart(5)
        } tok/s`,
    );
  }
  ...
  for (const ctx of [16, 128, 512]) {
    ...
    console.log(
      `    at ${String(ctx).padStart(3)} keys of context: ${ms.toFixed(2)} ms/token   ` +
  ...
  for (const K of [1, 4, 16, 64]) {
    ...
    console.log(
      `    K=${String(K).padStart(2)}: ${(ms / K).toFixed(2).padStart(6)} ms/token   ` +
  ...
  const n = 128;
  M.reset();
  const pre = await wall(async () => {
    M.reset();
    await M.step(fakeIds(n));
  }, 3);
  M.reset();
  ...
  console.log(
    `    ${n} tokens as decode steps: ${dec.toFixed(0)} ms   -> prefill is ` +
