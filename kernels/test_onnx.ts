// Diff the WGSL layers against web/shard0.onnx -- the engine flock uses today.
//
//   deno run --unstable-webgpu --allow-all --config kernels/deno.json kernels/test_onnx.ts
//
...
//
// This test SKIPS rather than fails when the shard or onnxruntime is missing:
// web/*.onnx are gitignored build artifacts regenerated by build_shards.py, so
// they are absent in a fresh clone or a worktree.
...
/** Where the shard and onnxruntime live. Both may be outside this worktree. */
function findPaths() {
  const candidates = [REPO, "/Users/ethanhinson/dev/flock/"];
  for (const root of candidates) {
    const shard = `${root}web/shard0.onnx`;
    const ort = `${root}node/node_modules/onnxruntime-node/dist/index.js`;
    try {
      Deno.statSync(shard);
      Deno.statSync(ort);
      return { shard, ort };
...
if (!paths) {
  console.log("  skip  web/shard0.onnx or onnxruntime-node not found.");
  console.log("        Both are build artifacts (gitignored); regenerate with");
  console.log("        build_shards.py and npm install in node/ to run this diff.");
  Deno.exit(0);
}
