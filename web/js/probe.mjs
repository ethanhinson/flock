...
export async function probe(gpu, nav) {
  ...
  let dev = null;
  try {
    // requestAdapter can HANG on some mobile drivers rather than rejecting,
    // which leaves a page on its initial "checking" text with no error anywhere.
    const ad = await Promise.race([
      gpu.requestAdapter(),
      new Promise((_, rej) => setTimeout(
        () => rej(new Error('requestAdapter did not answer within 10s')), 10000)),
    ]);
    ...
    // adapter.info THROWS on Safari builds without a flag, so it can never share
    // a try with anything else: it used to take the whole probe down on exactly
    // the devices whose limits we most need to read.
    try {
      detail.adapter = {vendor: ad.info?.vendor || null,
                        architecture: ad.info?.architecture || null,
                        device: ad.info?.device || null};
    } catch { detail.adapter = null; }
...
export function verdict(d) {
  ...
  if (!d.gpu) {
    // Wording matters here. "no GPU" sends people to inspect hardware that was
    // never the problem: every phone, tablet and laptop has one. What is absent
    // is the browser API. The two causes seen in practice are a Safari feature
    // flag and -- easy to miss -- Chrome refusing to expose WebGPU outside a
    // secure context, which is every plain-http LAN address.
    const insecureChrome = d.secureContext === false &&
      typeof navigator !== 'undefined' && /Chrome|Edg\//.test(navigator.userAgent);
    return {ok: false, headline: 'cannot be a bird',
            why: 'This browser is not exposing the WebGPU API. The GPU itself is ' +
                 'fine — a bird runs transformer layers as WebGPU compute shaders, ' +
                 'and there is nothing to fall back to.\n' +
                 (insecureChrome
                   ? 'Most likely cause: Chrome and Edge only expose WebGPU in a ' +
                     'secure context, and this page is plain http. Use Safari, or ' +
                     'serve the coordinator over https.'
                   : 'On iPad/iPhone before iPadOS 26: Settings > Apps > Safari > ' +
                     'Advanced > Feature Flags > WebGPU, then reload.')};
  }
