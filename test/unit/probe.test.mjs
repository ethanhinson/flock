...
console.log('\nno WebGPU -> says so, and does not pretend to have limits:');
{
  ...
  // Not "iOS 17": an iPad on 17.2 reported no navigator.gpu in this very project.
  // WebGPU ships ENABLED from iPadOS/iOS 26; on 17.x and 18.x it exists only
  // behind a Safari feature flag, so the actionable advice is the flag path.
  ok('  and says how to act on it', /Feature Flags|secure context/.test(v.why), v.why);
}
